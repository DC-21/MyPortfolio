    ...
    <div className="flex sticky shadow-md top-0 z-50 w-full px-4 md:px-8 py-4 items-center justify-between transition-all bg-white">
      <a className="flex items-center">
        <img
          src={logo}
          alt="Logo"
          className="h-[28px] w-[64px] hover:scale-105 transition-all"
        />
      </a>
      <ul className="md:flex hidden gap-6 text-slate-700">
        <li
          onClick={handleHome}
          className=" hover:border-b-[1px]  border-slate-500 transition-all duration-300 px-2 py-1"
        >
          <a>Home</a>
        </li>
        <li
      ...
      <button type="button" className="md:hidden block" onClick={handleMenu}>
        ...
        {open ? (
          <ul
            id="menu"
            className="absolute right-0 shadow-2xl items-center z-50 top-[3.7em] w-1/4 py-2 gap-2 flex flex-col text-left h-[7em] rounded bg-white"
          >
            <li onClick={handleHome} className="flex w-full text-left">
              <a className="w-full hover:bg-slate-300 pl-2 text-slate-700">
                Home
              </a>
            </li>
